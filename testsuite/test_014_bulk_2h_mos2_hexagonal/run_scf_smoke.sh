#!/usr/bin/env bash
# First fixed-cell acceptance gate for the six-atom 2H-MoS2 test.
set -euo pipefail
...
RUN_DIR="${RUN_DIR:-$SCRIPT_DIR/results/scf_smoke}"
...
grep -q "force: Total stress:" "$RUN_DIR/Conquest_out"
if grep -Eq "ERROR|CQ_ABORT|NaN" "$RUN_DIR/Conquest_out"; then
  echo "FAIL: error marker found in CONQUEST output" >&2
  exit 1
fi

echo "PASS: bulk 2H-MoS2 fixed-cell SCF converged with 52 electrons."
