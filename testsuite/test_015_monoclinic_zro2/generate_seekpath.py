#!/usr/bin/env python3
"""Generate an HPKOT path and a matching CONQUEST k-line block."""
...
def read_coords(filename: Path):
    ...
    lattice = [
        [float(value) * BOHR_TO_ANGSTROM for value in lines[index].split()[:3]]
...
def main():
    ...
    parser.add_argument("--conquest-block", type=Path, required=True)
    args = parser.parse_args()
    result = seekpath.get_path_orig_cell(
        read_coords(args.coords),
        recipe="hpkot",
    ...
    block = [
        f"Diag.NumKptLines            {len(result['path'])}",
        "%block Diag.KpointLines",
    ]
    for start, end in result["path"]:
        for label in (start, end):
            block.append(
                " ".join(f"{value:.12f}" for value in result["point_coords"][label])
            )
    block.append("%endblock")
    args.conquest_block.write_text("\n".join(block) + "\n", encoding="utf-8")
