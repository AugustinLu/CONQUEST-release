#!/usr/bin/env python3
"""Plot monoclinic ZrO2 bands and species/orbital-resolved pDOS."""
...
def main():
    ...
    dos_axis.plot(zirconium[:, 4], zirconium[:, 0], color="#7c3aed",
                  label="Zr d", linewidth=1.4)
    ...
    summary = {
        ...
        "valence_minus5_to_0ev_zr_d_weight": valence_zr_d,
        "conduction_0_to_5ev_o_p_weight": conduction_o_p,
        "conduction_0_to_5ev_zr_d_weight": conduction_zr_d,
    }
    ...
    if forces[-1] > 5.0e-4:
        raise ValueError(f"Atomic relaxation did not converge: {forces[-1]:.8f}")
    if valence_o_p <= valence_zr_d:
        raise ValueError("Expected O p character to dominate the upper valence band")
    if conduction_zr_d <= conduction_o_p:
        raise ValueError("Expected Zr d character to dominate the low conduction band")
